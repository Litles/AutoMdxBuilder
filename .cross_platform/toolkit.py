    ...
    if os.path.isfile(mfile) and mfile.endswith('.mdx'):
        ...
        if multi_mdd_flg and input('检查到目录下存在 mdd 分包, 是否全部解包 (Y/N): ') in ('Y', 'y'):
            ...
            for mdd_name in mdd_names:
                ...
                mdict(['-x', os.path.join(cur_dir, mdd_name), '-d', out_dir])
        else:
            mdict(['-x', mfile, '-d', out_dir])
    else:
...
def pack_to_mdict(file_final_txt, file_dict_info, dir_data, dir_output):
    ...
    if os.path.exists(file_final_txt) and os.path.exists(file_dict_info):
        # 给词条添加编号信息
        cur_path = os.getcwd()
        tmp_final_txt = os.path.join(os.path.join(cur_path, '_tmp'), 'tmp_final.txt')
        with open(file_final_txt, 'r', encoding='utf-8') as fr:
            with open(tmp_final_txt, 'w', encoding='utf-8') as fw:
                ...
                for line in fr:
                    ...
                    if (not link_flg) and re.match(r'^</>\s*$', line):
                        ...
                        link_flg = False
                    fw.write(line)
        mdict(['--description', file_dict_info, '--encoding', 'utf-8', '-a', tmp_final_txt, ftitle+'.mdx'])
    else:
        print(Fore.RED + "ERROR: " + Fore.RESET + f"文件 {file_final_txt} 或 {file_dict_info} 不存在")
        mdx_flg = False
    # 打包 mdd
    if dir_data is not None:
        mdd_flg = pack_to_mdd(dir_data, ftitle)
...
def pack_to_mdd(dir_data, ftitle):
    ...
    if pack_flg:
        ...
        if split_flg:
            ...
            for sds in pack_list:
                ...
                # 移完之后打包
                if mdd_rk == 0:
                    mdict(['-a', tmp_dir, ftitle+'.mdd'])
                else:
                    mdict(['-a', tmp_dir, f'{ftitle}.{str(mdd_rk)}.mdd'])
                # 打包完再移回去
...
def convert_pdf_to_imgs_fitz(file_pdf, dir_out, dpi=300):
